...
import { startOfWeek, endOfWeek, addDays, format, getDay, subWeeks, addWeeks, parseISO, isPast } from 'date-fns';
...
const TimesheetEntryPage = () => {
    ...
    const isEditable = !isPast(endOfWeek(weekStart)) || !!editId;
    const weekDates = Array.from({ length: 7 }).map((_, i) => addDays(weekStart, i));

    const dailyTotals = Array.from({ length: 7 }).fill(0);
            ...
            try {
                ...
                const transformedRows = data.projects.map(p => {
                    ...
                    p.entries.forEach(entry => {
                        ...
                        if (dayIndex !== -1) daily_hours[dayIndex] = entry.hours;
                    });
                    return { project_id: p.project_id, name: p.name, notes: p.notes || '', daily_hours };
                });
    ...
    const handleRemoveRow = (rowIndex) => {
        setTimesheetRows(timesheetRows.filter((_, index) => index !== rowIndex));
    };
    ...
    const handleConfirmSubmit = (status) => {
        if (timesheetRows.length > 0 && timesheetRows.flatMap(r => r.daily_hours).reduce((a, b) => a + b, 0) === 0) {
            setSnackbar({ open: true, message: 'Timesheet has projects but no hours.', severity: 'warning' });
            return;
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editId) await api.put(`/timesheet/${editId}`, payload);
            else await api.post('/timesheet', payload);
    ...
    return (
        <Paper sx={{ p: 3 }}>
            ...
            {!isEditable && !editId && (
                <Alert severity="warning" sx={{mb: 2}}>You can only enter time for the current or future weeks. Past weeks are view-only.</Alert>
            )}
            <TableContainer>
                <Table>
                    <TableHead>
                    ...
                    <TableBody>
                        {timesheetRows.map((row, rowIndex) => (
                            <TableRow key={rowIndex}>
                                ...
                                <TableCell>
                                    <IconButton onClick={() => openNotesDialog(rowIndex)} color={row.notes ? 'primary' : 'default'} disabled={!isEditable}>
                                        <NotesIcon />
                                    </IconButton>
                                </TableCell>
                                {row.daily_hours.map((hours, dayIndex) => (
                                    <TableCell key={dayIndex}>
                                        <TextField type="number" size="small" inputProps={{ step: 0.25, min: 0, style: { textAlign: 'center' } }} sx={{ width: '80px' }} value={hours || ''} onChange={(e) => handleHourChange(rowIndex, dayIndex, e.target.value)} disabled={!isEditable} />
                                    </TableCell>
                                ))}
...
export default TimesheetEntryPage; // <-- THIS LINE IS CRUCIAL